...
//	}
	public static void main(String[] args) {
		JSONArray obj = null;
		try {
			obj = (JSONArray) new JSONParser().parse(new FileReader("C:\\Users\\clair\\Documents\\EngineeringEssentials\\caseStudy\\services\\src\\main\\java\\utility\\historicalStockData.json"));
		} catch (IOException e) {
			System.out.println("Error 1");
			return;
		} catch (ParseException e) {
		...
		for (int i = 0; i < obj.size(); i++) {
			...
			for(Map.Entry<String, Object> entry : ((HashMap<String,Object>) innerObj.toArray()[0]).entrySet()) {
			    ...
			    System.out.println("val: " + realValue);
